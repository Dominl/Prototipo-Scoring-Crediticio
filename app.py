...
with col1:
    ...
    edad = st.number_input("Edad del cliente", min_value=18, max_value=100, value=35,
                           help="Edad actual del cliente en años")
    nro_dependiente = st.number_input("Número de dependientes", min_value=0, max_value=20, value=1,
                                      help="Personas que dependen económicamente del cliente")

    st.markdown("#### 💰 Datos Financieros")
    mto_ingreso = st.number_input("Monto de ingreso mensual (S/)", min_value=0.0, value=3000.0, step=100.0,
                                  help="Ingreso mensual del cliente en soles")
    prct_uso_tc = st.slider("Porcentaje de uso de tarjeta de crédito el último mes", 0.0, 1.0, 0.3, 0.01,
                            help="0 = no usó la tarjeta / 1 = usó el 100% del límite")
    prct_deuda = st.slider("Ratio de deuda financiera vs ingresos del cliente", 0.0, 1.0, 0.2, 0.01,
                           help="0 = sin deuda / 1 = deuda igual a sus ingresos")

with col2:
    st.markdown("#### 🏦 Historial Crediticio")
    nro_prod = st.number_input("Nro. de préstamos y tarjetas de crédito (último año)", min_value=0, max_value=20, value=2,
                               help="Incluye créditos vehiculares, hipotecarios y tarjetas")
    nro_cred_hip = st.number_input("Número de créditos hipotecarios", min_value=0, max_value=10, value=0,
                                   help="Cantidad de créditos hipotecarios activos")

    st.markdown("#### ⚠️ Historial de Retrasos")
    nro_retraso3 = st.number_input("Nro. de préstamos con retraso mayor a 3 meses (últ. 3 años)",
                                   min_value=0, max_value=20, value=0,
                                   help="Antes llamado: Nro_prestao_retrasados")
    nro_retraso2 = st.number_input("Nro. de veces con retraso mayor a 2 meses (últ. 3 años)",
                                   min_value=0, max_value=20, value=0,
                                   help="Antes llamado: Nro_retraso_60dias")
    nro_retraso1 = st.number_input("Nro. de retrasos en pagos mayor a 1 mes (últ. 3 años)",
                                   min_value=0, max_value=20, value=0,
                                   help="Antes llamado: Nro_retraso_ultm3anios")
